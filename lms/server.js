...
app.get('/dashboard', (req, res) => {
    if (!req.session.user) {
        return res.status(401).send('You must be logged in to view the dashboard');
    }
...
// Route to select a course for the logged-in user
app.post('/select-course', (req, res) => {
    if (!req.session.user) {
        return res.status(401).send('You must be logged in to select a course');
    }
...
app.get('/selected-courses', (req, res) => {
    if (!req.session.user) {
        return res.status(401).send('You must be logged in to view selected courses');
    }
